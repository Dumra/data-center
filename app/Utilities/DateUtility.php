...
class DateUtility
{
    ...
    public static function getRange($startTime, $endTime)
    {
        if (is_null($endTime)) {
            return [self::formatDate($startTime), self::getNowTime()];
        }

        return [self::formatDate($startTime), self::formatDate($endTime)];
    }
